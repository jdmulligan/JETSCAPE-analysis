...
This base file is derived from the base file for the STAT analysis, `analyze_events_base_STAT.py`.
...
################################################################
class AnalyzeJetscapeEvents_BaseJetConstituents(common_base.CommonBase):
    ...
    def __init__(self, config_file="", input_file="", output_dir="", **kwargs):
        ...
        if self.is_AA:
            ...
            if "Run" in _final_state_hadrons_path.stem:
                # This is based on the STAT production convention
                ...
                # If we're not using the RunXXX format, we need to specify the centrality range in the config file.
                # e.g. `centrality_range: [0, 10]`.
                self.centrality = config["centrality_range"]
    ...
    def write_output_objects(self):
        # Write the data and the cross section to parquet
        self.output_dataframe = ak.zip({
                "data": ak.Array(self.output_event_list),
                "cross_section": ak.Array([self.cross_section_dict])
            },
            depth_limit = 1
        )
        ...
        ak.to_parquet(
            ...
            destination=str(self.output_dir / self.output_file),
            compression="zstd",
            # Let parquet select the best value
